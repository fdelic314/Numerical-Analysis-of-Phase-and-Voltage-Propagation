...
V = np.load("Voltage 1D.npy")
f = np.load("Frequency 1D.npy")
...
x_P = 50 # Starting position

fig1 = plt.figure(figsize=(7,5))
legend_properties = {'weight':'bold'}
plt.plot(tarray,V[:, int(x_P)],tarray,V[:, int(x_P + 5)],tarray,V[:, int(x_P + 20)])
...
#fig1.savefig('Voltage displacement.png', dpi=150, bbox_inches= 'tight')

# Obtain analytical solution of decay
max_dev = np.max(f[:,int(x_P)])
f_anal = 0.06 * np.exp(-0.15 * (tarray - 0.85))

fig2 = plt.figure(figsize=(7,5))
legend_properties = {'weight':'bold'}
plt.plot(tarray,f[:, int(x_P)],tarray,f[:, int(x_P + 5)],tarray,f[:, int(x_P + 20)], tarray, f_anal)
plt.ylabel("Frequency deviation, $w_i$ (p.u.)", fontsize = 15,fontweight='bold')
plt.xlabel("$t - t_0$ (s)", fontsize = 15,fontweight='bold')
plt.legend(['$dist=0$','$dist=5$','$dist=20$', "analytical"],prop=legend_properties)
plt.xticks(fontweight='bold',fontsize=12)
plt.yticks(fontweight='bold',fontsize=12)
#fig2.savefig('Frequency displacement.png', dpi=150, bbox_inches= 'tight')
...
# Analytical function
a = np.array(tarray)
b = V[slice_start + 100, x_P] * np.exp(-0.1 * a)
#b = 0.0000692 * np.exp(-0.1 * a) * a**(-0.5)
...
plt.legend(['Simulated data','$\exp ( B \cdot t )$'],prop=legend_properties)
plt.xticks(fontweight='bold',fontsize=12)
plt.yticks(fontweight='bold',fontsize=12)
plt.grid(True, which="both")
#fig3.savefig('Loglog voltage.png', dpi=150, bbox_inches= 'tight')
...
plt.xticks(fontweight='bold',fontsize=12)
plt.yticks(fontweight='bold',fontsize=12)
#fig4.savefig('Voltage variance.png', dpi=150, bbox_inches= 'tight')
...
plt.xticks(fontweight='bold',fontsize=12)
plt.yticks(fontweight='bold',fontsize=12)
#fig5.savefig('Frequency variance.png', dpi=150, bbox_inches= 'tight')
...
plt.xticks([0,25,50,75,99],[0,25,50,75,100], fontweight='bold',fontsize=12)
...
plt.xlabel("Nodes", fontsize = 15,fontweight='bold')
plt.colorbar()
plt.grid(None)
#fig12.savefig('Voltage petrubation heat map', dpi=150, bbox_inches= 'tight')
...
plt.colorbar()
plt.grid(None)
#fig13.savefig('Frequency petrubation heat map', dpi=150, bbox_inches= 'tight')
